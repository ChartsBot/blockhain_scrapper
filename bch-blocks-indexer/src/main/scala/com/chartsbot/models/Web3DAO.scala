...
trait Web3DAO {
  ...
  def getBlock(blockNumber: Long): Future[EthBlock.Block]
...
class DefaultWeb3DAO @Inject() (web3Connector: Web3Connector)(implicit val ec: ExecutionContext) extends Web3DAO {
  ...
  val web3Client: Web3j = web3Connector.web3Eth

  def getBlock(blockNumber: Long): Future[EthBlock.Block] = {
    val blockParameterNumber = new DefaultBlockParameterNumber(blockNumber)
    web3Client.ethGetBlockByNumber(blockParameterNumber, false).sendAsync().toScala.map(_.getBlock)
  }
